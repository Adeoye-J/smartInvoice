          ...
          {steps.map((step, index) => (
            <div key={index} className="relative">
              {index < steps.length - 1 && (
                <div className="hidden md:block absolute top-12 left-full w-full h-0.5 bg-linear-to-r from-blue-200 to-transparent -translate-x-1/2"></div>
              )}
              <div className="relative bg-linear-to-br from-blue-50 to-white p-8 rounded-2xl border border-blue-100 hover:shadow-xl transition-all duration-300">
                <div className="absolute -top-4 -left-4 w-12 h-12 bg-linear-to-br from-blue-600 to-blue-700 text-white rounded-xl flex items-center justify-center text-lg font-bold shadow-lg">
                  {step.number}
                </div>
